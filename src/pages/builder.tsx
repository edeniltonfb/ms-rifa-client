...
import {
    ...
    FormLabel,
    Paper,
    // Grid, // Removendo importação não utilizada, como sugerido em seu comentário
    TextField,
    CircularProgress as MuiCircularProgress,
    Link, // <--- NOVA IMPORTAÇÃO: Link para exibir o download
} from '@mui/material';
...
// --- Interface para a resposta da API de impressão de teste (nova) ---
...
const Builder: React.FC = () => {
    ...
    // --- NOVO ESTADO: Para armazenar o link de download do teste ---
    ...
    const handleSubmitForm = async () => { // Tornada async
        ...
        setTestPdfLink(null); // Limpa o link de download anterior ao configurar um novo layout
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        setTestPdfLink(null); // Limpa o link de download ao mover um item
    ...
    // --- FUNÇÃO AUXILIAR REFATORADA PARA CHAMADAS À API DE IMPRESSÃO (AGORA EXIBE O LINK) ---
    const handlePrintApiCall = async (
        ...
        setTestPdfLink(null); // Limpa o link de download anterior ao fazer nova requisição
        ...
        try {
            ...
            //let url = `http://127.0.0.1:8082/msrifaadmin/api/${endpointSuffix}?token=${user.token}`; // Use a URL de produção ou remova o comentário para teste local
            ...
            // SEMPRE espera uma resposta JSON agora
            const result: PrintTestApiResponse = await response.json(); // Tipando a resposta
            ...
            if (result.success) {
                if (endpointSuffix === 'gerararquivotesteimpressao') {
                    ...
                    // --- Lógica EXISTENTE para 'gerararquivoimpressao' ---
                    enqueueSnackbar('Arquivo de impressão gerado com sucesso!', { variant: 'success' });
                    setCodigo('');
                }
            } else {
                // Lógica de erro geral para ambos os endpoints
                ...
                <Box sx={{ mt: 0 }}>
                    ...
                    <Box sx={{ mt: 0, mb: 2 }}> {/* Adicionado mb para espaçamento com o painel */}
                        <Button
                            ...
                            onClick={() => {
                                setPanelConfig(null);
                                setTestPdfLink(null); // Limpa o link ao resetar o layout
                            }}
                        >
                    ...
                    <Paper sx={{ p: 2, mt: 4 }}>
                        ...
                        {testPdfLink && (
                            <Box sx={{ mt: 2 }}>
                                ...
                                <Link
                                    href={testPdfLink || ''}
                                    download="layout_teste.pdf" // Sugere um nome de arquivo para download
                                    target="_blank" // Abre em nova aba, mas com o atributo download, tende a baixar.
