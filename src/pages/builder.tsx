...
const Builder: React.FC = () => {
    ...
    const handleSubmitForm = () => {
        ...
        for (let i = 0; i < calculatedNumElements; i++) {
            const randomX = Math.random() * (currentPanelWidth - DRAGGABLE_ITEM_WIDTH);
            const randomY = Math.random() * (currentPanelHeight - DRAGGABLE_ITEM_HEIGHT);
    ...
    const handlePrintApiCall = async (
        endpointSuffix: string, // Ex: 'gerararquivoimpressao' ou 'gerararquivotesteimpressao'
        requiresCodigoValidation: boolean // True se o código é obrigatório e vai na URL
    ) => {
        // Validações Comuns
        if (!user?.token) {
        ...
        // Validação do Código (Condicional)
        ...
        // --- Montar a lista de posições para o corpo da requisição (lógica comum) ---
        ...
        try {
            ...
            if (requiresCodigoValidation) { // Adiciona o código se necessário
                url += `&codigo=${codigo}`;
            }
            ...
            if (result.success) {
                // Mensagens de sucesso são iguais, mas poderiam ser passadas como parâmetro se necessário
                enqueueSnackbar('Arquivo de impressão gerado com sucesso!', { variant: 'success' });
                setCodigo(''); // Limpa o campo após o sucesso, comportamento comum a ambos
            } else {
    ...
    // --- As funções originais agora chamam a auxiliar ---
    const handleSubmitPrintFileTest = async () => {
        await handlePrintApiCall('gerararquivotesteimpressao', false); // Não exige código
    };

    const handleSubmitPrintFile = async () => {
        await handlePrintApiCall('gerararquivoimpressao', true); // Exige código e o inclui na URL
    };
